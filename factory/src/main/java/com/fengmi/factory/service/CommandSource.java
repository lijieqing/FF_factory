...
public class CommandSource implements GlobalCommandReceiveListener {
    ...
    private final BroadcastReceiver fakeCommandReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action != null && action.equals(FAKE_COMMAN_ACTION)) {
                ...
                if (para1 != null) {
                    if (para1.startsWith("file://")) {
                        try {
                            File file = new File(para1.replace("file://", "/"));
                            if (file.exists()) {
                                ...
                                for (byte datum : data) {
                                    par.append(datum & 0xFF).append(",");
                                }
                                para1 = par.toString();
                            } else {
                                Log.e(TAG, "para1 is not illegal,para1 is " + para1);
                                return;
                            }
    ...
    public CommandSource(Context context, OnCommandListener listener) {
    ...
    private void initCP210x(Context context) {
        if (cp210xUsb == null) {
            cp210xUsb = new USB.USBBuilder(context)
                    .setBaudRate(115200)
                    .setDataBits(UsbSerialPort.DATABITS_8)
