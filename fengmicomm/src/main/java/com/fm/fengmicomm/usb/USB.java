...
public class USB {
    ...
    private int VID;
    private int PID;
    // FTDI   mVendorId=1027,mProductId=24577
    // CP210X mVendorId=4292,mProductId=60000
    private static int[][] USB_PORT_INFO = new int[][]{
            //{VID,PID},
            {1027, 24577},
            {4292, 60000}
    };
    ...
    private static boolean isTargetUsb(UsbDevice device) {
        if (device != null) {
            int vid = device.getVendorId();
            int pid = device.getProductId();
            for (int i = 0; i < USB_PORT_INFO.length; i++) {
                int[] PORT = USB_PORT_INFO[i];
                if (PORT[0] == vid && PORT[1] == pid) {
                    return true;
                }
            }
            return false;
